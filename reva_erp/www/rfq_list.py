...
def get_context(context, **dict_params):
    ...
    get(
        doctype="Request for Quotation",
        txt=frappe.local.form_dict.get("txt"),
        limit_start=frappe.local.form_dict.get("limit_start"),
        pathname=frappe.local.form_dict.get("pathname"),
    )
)
...
STATUS_MAP = {
	"Submitted": "Sent to Supplier",   # change to whatever you want
...
def get_list_data(
	...
	# ---- Custom Supplier Quotation Status ----

	# get the supplier linked with logged-in user
	supplier = frappe.db.get_value("Supplier", {"email_id": frappe.session.user}, "name")
	for d in raw_result:
		d_status = "Create Quotation"  # default
		d.indicator_color = "orange" 

		# Find all SQ Items linked to this RFQ
		sq_parents = frappe.db.get_all(
			"Supplier Quotation Item",
			filters={"request_for_quotation": d.name},
			fields=["parent"]
		)

		# If any supplier quotation exists for this RFQ
		for row in sq_parents:
			sq_supplier = frappe.db.get_value("Supplier Quotation", row.parent, "supplier")
			
			if sq_supplier == supplier:
				d_status = "Quotation Created"
				d.indicator_color = "green" 
				break

		# override status field
		d.status = d_status

    # --- End Custom Status Mapping ---
...
def set_route(context):
	...
	if context.web_form_name:
		context.route = f"rfq?name={quoted(context.doc.name)}"
	elif context.doc and getattr(context.doc, "route", None):
		context.route = context.doc.route
	else:
		context.route = f"rfq/{quoted(context.doc.name)}"
