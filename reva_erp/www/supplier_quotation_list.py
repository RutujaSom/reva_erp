...
def get(doctype, txt=None, limit_start=0, limit=20, pathname=None, **kwargs):
	...
	for doc in raw_result:
		...
		if not frappe.flags.in_test:
			new_context["pathname"] = "supplier-quotations"
...
def get_list_data(
	...
	for d in raw_result:
		...
		# Default values
		custom_status = "Quotation Submitted"
		d.indicator_color = "grey" 

		# Fetch workflow_state & docstatus
		workflow_state = getattr(d, "workflow_state", None)
		docstatus = getattr(d, "docstatus", None)

		# 1️⃣ If workflow_state = Approved
		if workflow_state == "Approved":
			custom_status = "Quotation Approved"
			d.indicator_color = "green" 

		d.status = custom_status

	# ---- End Custom Supplier Quotation Status ----
...
def set_route(context):
	...
	if context.web_form_name:
		context.route = f"supplier-quotations?name={quoted(context.doc.name)}"
	elif context.doc and getattr(context.doc, "route", None):
		context.route = context.doc.route
	else:
		context.route = f"supplier-quotations/{quoted(context.doc.name)}"
