...
frappe.pages['supplier-quotation-a'].on_page_load = function(wrapper) {
    ...
    //     data.forEach(row => {
    //         console.log("reow .....",JSON.stringify(row))
    //         let tr = $('<tr></tr>');
    ...
    async function render_table(data) {
        ...
        for (let row of data) {
            let quotation = row.quotation;

            // 👉 Fetch workflow state
            let ws = await frappe.db.get_value("Supplier Quotation", quotation, "workflow_state");
            console.log("ws ....",ws)
            // 👉 Skip non-pending
            if (!ws || ws.message.workflow_state !== "Pending") {
                continue;
            }

            alert("Row Processing: " + JSON.stringify(row));

            let tr = $('<tr></tr>');
            columns.forEach(c => tr.append('<td>' + (row[c] ?? "") + '</td>'));

            let toggle = $(`
                <div class="toggle-switch reject" data-name="${quotation}">
                    <div class="toggle-slider"></div>
                    <div class="toggle-label approve-label">Approve</div>
                    <div class="toggle-label reject-label">Reject</div>
                </div>
